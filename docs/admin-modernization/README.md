...
## Điều kiện chuyển đề xuất thành triển khai

Một hạng mục chỉ được chuyển từ hồ sơ thiết kế sang code khi đã xác định:

1. Business rule và vai trò được phép thao tác.
2. Schema hoặc dữ liệu liên quan.
3. API request/response và mã lỗi.
4. Màn hình, trạng thái loading/empty/error và hành vi xác nhận.
5. Acceptance test có thể tái hiện.
6. Ảnh hưởng tương thích với endpoint hoặc màn hình hiện có.

Sau khi triển khai, cập nhật tài liệu hiện hành ngoài hồ sơ này và ghi rõ phần nào của đề xuất đã được áp dụng, thay đổi hoặc loại bỏ.
